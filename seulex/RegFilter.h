...
/*
    ...
    providing functions to convert rule definitions and complex regular expressions 
    into regular expressions containing only simple rules;
*/
#include <string>
#include <map>
	...
	static std::string spcharFilter(std::string src) {
		...
		for (int i = 0; i < srcLen; ++i) {
			...
			if (c == '\\' && i + 1 < srcLen) {
				...
				c = src[i];
				switch (c)	{
                // The formula cannot contain Spaces, if you want to represent Spaces,
                // use the escape way we define '\p' to indicate;
				case 'p':
					c = ' ';              //spec rule
					break;
            ...
            // {}[] is replaced with characters outside the special allowed character set
			switch (c){
			case '{':
				c = Brace_left;
				break;
