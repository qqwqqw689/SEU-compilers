	...
	for (string::const_iterator it = exp.begin(); it != exp.end(); ++it) {
		...
		// End of bracket
			...
			for (auto c = inBracket.begin(); c != inBracket.end(); ++c) {
				if (first) {					// Add | between all but the first
					first = false;
				}
				else {
					res.push_back(to_operator('|'));
				}
				if (*c == '\\') {				// If it is escaped, the escaped symbol is viewed as a whole
					if (need_escape(*(++c))) {
						res.push_back(*c);
					}
					else {
...
int ParseLexFile(ifstream& ifs, ofstream& ofs) {
...

	{
		...
		*/
		...
		while (getline(ifs, line)) {						// First find two %% and divide the file into three parts
			++lineCount;
